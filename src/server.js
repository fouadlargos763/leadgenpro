...
const { generateEmailContent, sendFollowUpEmail, sendOutreachEmail } = require('./mailer');
const { generatePersonalizedHook } = require('./aiPersonalizer');
const { generateLeadInsight, optimizeEmail } = require('./aiInsights');
const { analyzeWebsite } = require('./websiteAnalyzer');
const { registerUser, loginUser, requireAuth, requirePageAuth, requireAdmin, loadUsers, saveUsers, findUserById } = require('./auth');
const { createCheckoutSession, handleWebhookEvent, getPlanLimits, cancelSubscription, getSubscription, setSubscription, PLANS } = require('./billing');
const { incrementUsage, getUsage } = require('./usage');
const { trackEvent, getAnalyticsMetrics, getRevenueMetrics } = require('./analytics');
const { logReferral, getAllReferrals, getUserReferrals, getBonusLeads } = require('./referrals');
const {
    logger,
...
const { sendWelcomeEmail, sendSubscriptionConfirmation, sendPaymentReceipt } = require('./notifications');
    ...
    if (process.env.EMAIL_HOST && process.env.EMAIL_USER) {
        ...
        validEmails.forEach(email => {
            transporter.sendMail({
                ...
                to: email,
                subject: `Invitation from ${user.name} | LeadGenPro`,
                text: `Hey, I've been using LeadGenPro to scale my outreach and thought you might like it. Join me using my link: ${referralLink}`
            }).catch(err => console.error('Failed to send invite email:', err));
        });
    } else {
        validEmails.forEach(email => {
            console.log(`[MOCK EMAIL] To: ${email} | Subject: Join LeadGenPro | Body: Link: ${referralLink}`);
        });
    }
...
app.get('/api/campaigns', requireAuth, (req, res) => {
    ...
    const files = fs.readdirSync(userDir)
        .filter(f => f.startsWith('leads_') || f.startsWith('enriched_leads_'))
        .sort().reverse();
...
app.get('/api/campaign-stats', requireAuth,
    ...
    const getFiles = (dir) => {
        ...
        return fs.readdirSync(dir)
                .filter(f => f.startsWith('leads_') || f.startsWith('enriched_'))
                .map(f => ({ name: f, dir }));
    };

    const files = [...getFiles(userDir), ...getFiles(dataDir)];
    ...
    const campaigns = finalFiles.map(f => {
        try {
            const leads = JSON.parse(fs.readFileSync(path.join(f.dir, f.name), 'utf8'));
            ...
            let name = f.name.replace('enriched_leads_', '').replace('leads_', '').replace('.json', '').replace(/_/g, ' ');
            let match = f.name.match(/_([0-9]+)\.json$/);
            if (match && name.length < 5) {
            ...
            return { name, file: f.name, total, sent, opens, openRate: sent > 0 ? Math.round((opens / sent) * 100) : 0, status };
        } catch (e) {
            return null;
        }
    }).filter(Boolean);
    
    // Only include fallback from root if user is admin or has no campaigns yet
    if ((req.user.role === 'admin' || campaigns.length === 0) && fs.existsSync(dataDir)) {
        const rootFiles = fs.readdirSync(dataDir, { withFileTypes: true })
            .filter(f => !f.isDirectory() && (f.name.startsWith('leads_') || f.name.startsWith('enriched_')));
        
        rootFiles.forEach(f => {
            if (campaigns.find(c => c.file === f.name)) return;
            try {
                const data = JSON.parse(fs.readFileSync(path.join(dataDir, f.name), 'utf8'));
                const total = data.length;
                const name = f.name.replace('enriched_leads_', '').replace('leads_', '').replace('.json', '').replace(/_/g, ' ');
                campaigns.push({ name, file: f.name, total, sent: 0, opens: 0, openRate: 0, status: 'Shared' });
            } catch (e) {}
        });
    }
...
/**
 * API: Update a lead's data (USER-SCOPED)
 */
app.post('/api/leads/update', requireAuth, (req, res) => {
    const uid = req.user.id;
    const { leadId, file, updates } = req.body;
    if (!leadId || !file || !updates) return res.status(400).json({ error: 'Missing parameters' });
    ...
    try {
        let leads = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        const index = leads.findIndex(l => (l.id === leadId) || (l.title === leadId) || (l.name === leadId));

        if (index === -1) return res.status(404).json({ error: 'Lead not found in campaign' });
        ...
        if (updates.status) {
            logActivity(uid, leadId, 'status_change', { newStatus: updates.status });
        }
...
app.get('/api/all-clients', requireAuth, (req, res) => {
    ...
    const findLeadsInDir = (dir) => {
        ...
        try {
            ...
            console.error(`[Error] Failed to read dir ${dir}:`, err.message);
        }
    };

    // Aggregate from user's place
    findLeadsInDir(userDir);
    
    // Aggregate from root only if Admin or if they have nothing else
    if (req.user.role === 'admin' || allClients.length === 0) {
        findLeadsInDir(dataDir);
    }

    console.log(`[API] all-clients for ${uid}: Found ${allClients.length} leads in ${processedFiles.size} files.`);
...
/**
 * API: Add a new lead manually (USER-SCOPED)
 */
app.post('/api/leads/add', requireAuth, (req, res) => {
    const uid = req.user.id;
    const { name, email, status, phone, website, notes } = req.body;
    if (!name) return res.status(400).json({ error: 'Company Name is required.' });

    const userDir = getUserDataDir(uid);
    const fileName = 'leads_manual_additions.json';
...
app.post('/api/action', requireAuth, leadsLimiter, deduplicateSearch, (req, res) => {
    const { action, category, location, campaignName, maxLeads, campaignFile } = req.body;
    const uid = req.user.id;
    const limits = getPlanLimits(uid);
    ...
    // AI Personalization Limitation - Warning only, don't block
    if (action === 'enrich' || action === 'mail') {
        if (!limits.features.ai && limits.plan === 'free') {
            logger.info('AI feature requested on free plan - proceeding without AI enrichment.');
            // We let them proceed, but the internal logic should handle bypass
        }
    }

    // Automation Limitation (Queueing) - Allow on Free plan but without mass-automation
    if (action === 'send') {
        if (!limits.features.automation && limits.plan === 'free') {
            logger.info('Automation requested on free plan - allowing single batch send.');
        }
    }

    const actionMap = {
        'find': ['index.js', 'find'],
        'enrich': ['index.js', 'enrich'],
        'mock': ['index.js', 'mock'],
        'mail': ['index.js', 'mail'],
        'send': ['index.js', 'send'],
        'mock-find': ['index.js', 'mock-find']
    };
    ...
    if (action === 'find') {
        ...
        args.push(campaignName || `${catClean} in ${locClean}`);
        args.push((maxLeads || 50).toString());
    } else if (campaignFile) {
        args.push(campaignFile);
    }
    ...
    cmd.on('close', (code) => {
        ...
        if (action === 'find' && code === 0) {
            const match = stdoutBuffer.match(/Found (\d+) total businesses/) || stdoutBuffer.match(/Created (\d+) fallback leads/);
            if (match) {
...
// Bulk Analysis replaced by specialized endpoints if needed
...
/**
 * Activity Logging Helper (USER-SCOPED)
 */
function logActivity(uid, leadId, type, metadata = {}) {
    try {
        const userDir = getUserDataDir(uid);
        const activityFile = path.join(userDir, 'lead_activity.json');
        let activities = [];
        if (fs.existsSync(activityFile)) {
            try {
                activities = JSON.parse(fs.readFileSync(activityFile, 'utf8'));
